...
# input_folder = "./dataset_pny/"
# output_folder = "./dataset/images/train/"
# os.makedirs(output_folder, exist_ok=True)

# # Facteur d'agrandissement pour YOLOv8 (32x24 -> 320x240)
# SCALE_FACTOR = 10
...
#         # 1. Charger le tableau numpy (32x24)
#         img_np = np.load(filepath)
        
#         # 2. Extraire le minimum et le maximum de cette image spécifique
#         t_min = np.min(img_np)
#         t_max = np.max(img_np)
        
#         # 3. Appliquer la normalisation locale (0-255)
#         if t_max == t_min:
#             # Sécurité : si l'image a une température parfaitement uniforme
#             img_normalized = np.zeros_like(img_np, dtype=np.uint8)
#         else:
#             # Mise à l'échelle dynamique
#             img_normalized = ((img_np - t_min) / (t_max - t_min) * 255.0).astype(np.uint8)
        
#         # 4. Redimensionner l'image sans lisser les pixels (garder l'aspect thermique brut)
#         large_img = cv2.resize(img_normalized, 
#                                (32 * SCALE_FACTOR, 24 * SCALE_FACTOR), 
#                                interpolation=cv2.INTER_NEAREST)
        
#         # 5. Enregistrer le résultat
#         base_name = filename.replace('.npy', '.png')
#         cv2.imwrite(os.path.join(output_folder, base_name), large_img)

# print("Conversion terminée : Chaque image a été normalisée avec son propre Min/Max.")
...
# Tout ce qui est à 15°C ou moins sera noir (0)
# Tout ce qui est à 45°C ou plus sera blanc pur (255)
